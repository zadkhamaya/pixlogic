                ...
                <section className=' p-5 m-auto'>
                    ...
                    <div className=' grid grid-cols-2 gap:1 lg:grid-cols-3 lg:gap-3'>
                        {eventList && eventList.map(({ id, image, name: eventName, description, date, location, participants }) => {
                            return <EventCard key={id} id={id} image={image && image.startsWith("http") ? image : urlImage} name={eventName} description={description} date={date} location={location} participants={participants}></EventCard>
                        })}
                    </div>
                </section>
